...
- Security knobs: TLS (manual certs or Certbot), optional auth token, IP allowlist, rate limits, request-size limits, strict state-dir perms, audit log, chroot/privilege drop/unshare net (Unix).
...
- Hardened defaults: loopback binding, TLS enforcement on non-loopback, optional auth token, rate limits, and strict state-dir perms.
...
## Quick start
```bash
# build the binary (output: target/release/docdexd)
cargo build --release
# or install into ~/.cargo/bin
cargo install --path .
# Note: schema changes (like non-tokenized doc_ids) require reindexing existing repos:
#   rm -rf <repo>/.docdex/index && docdexd index --repo <repo>
...
- Serve with watcher: `docdexd serve --repo <path> --host 127.0.0.1 --port 46137 --log warn`.
- Secure serving: add `--auth-token <token>`; use TLS with `--tls-cert/--tls-key` or `--certbot-domain <domain>`.
...
- `--auth-token <token>` / `DOCDEX_AUTH_TOKEN`: optional bearer token required for HTTP requests.
- `--allow-ip a,b,c` / `DOCDEX_ALLOW_IPS`: optional comma-separated IPs/CIDRs allowed to reach the HTTP API (default: allow all).
...
- `--rate-limit-per-min <n>` / `DOCDEX_RATE_LIMIT_PER_MIN`: optional per-IP request budget per minute (0 disables).
...
- `--unshare-net` / `DOCDEX_UNSHARE_NET=true`: (Unix) unshare the network namespace before serving (requires CAP_SYS_ADMIN/root).
...
- `self-check --repo <path> --terms "foo,bar" [--limit 5]` — scan the index for sensitive terms before enabling access (fails with non-zero exit if any are found; reports sample hits and if more exist).
...
## Troubleshooting
- Missing binary: run `bash scripts/docdex/build.sh` or `cargo build --release`.
