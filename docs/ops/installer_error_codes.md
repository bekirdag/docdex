...
| `DOCDEX_DOWNLOAD_FAILED` | 20 | Download failed for the selected asset (non-404 HTTP status or transport failure). | `true/false` (printed when available) | Check network/proxy/firewall; retry; set `DOCDEX_GITHUB_TOKEN` if rate limited; configure `HTTPS_PROXY`/`HTTP_PROXY`/`NO_PROXY` if required; verify `DOCDEX_DOWNLOAD_REPO` and (if set) `DOCDEX_DOWNLOAD_BASE`. | Ensure release assets are publicly readable (or document token usage for private releases). |
| `DOCDEX_PERMISSION_DENIED` | 25 | Installer could not write to the temp directory or install location (EACCES/EPERM/EROFS). | N/A | Ensure the install path and temp dir are writable; use a user-writable npm prefix for global installs; avoid read-only filesystems. | Document required write permissions for install paths; avoid packaging on read-only volumes. |
