...
* **docdexd browser list/setup/install**  
  * **Description:** Lists discovered headless-capable browsers, runs auto-discovery, and installs a Docdex-managed Chromium build when no browser is present (macOS/Windows/Linux).  
  * **Usage:** docdexd browser list \| docdexd browser setup \| docdexd browser install  
  * **Behavior:** Persists `web.scraper.chrome_binary_path` and `web.scraper.browser_kind` in `~/.docdex/config.toml`; `web.scraper.auto_install` defaults to true (override with `DOCDEX_BROWSER_AUTO_INSTALL=0`). Chromium installs land under `~/.docdex/state/bin/chromium/`.
...
* **Behavior:**  
  ...
  * The npm installer uses fixed port `28491`, updates `~/.docdex/config.toml`, and injects MCP config into supported clients. It does not start the daemon during npm install; OS startup registration happens during `docdex setup`. If the port is already in use, installation still succeeds and configs are updated, but daemon startup may fail until the port is freed or reconfigured.
  * The npm installer **does not** install Ollama/models directly. It launches a setup wizard (`docdex setup`) that first detects supported local LLM services/models and only requests consent before installing the optional Ollama fallback or pulling fallback models.
  * When the user chooses the Ollama fallback and Ollama is installed but not running, the setup wizard will attempt to start it before pulling models.
  * Setup overrides: `DOCDEX_SETUP_FORCE` plus fallback-specific `DOCDEX_OLLAMA_INSTALL`, `DOCDEX_OLLAMA_MODEL_PROMPT`, `DOCDEX_OLLAMA_MODEL_ASSUME_Y`.
