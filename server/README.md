...
- Web discovery is enabled for the hosted daemon, including encrypted-repo
  ...
  Chromium under `/mnt/docdex/state` so link fetches work. Hosted discovery
  defaults to `https://se.overrid.com/search` first, local DuckDuckGo second,
  and paid providers such as Brave only after those free paths fail.
