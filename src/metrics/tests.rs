...
fn render_prometheus_includes_counters() {
    ...
    metrics.record_conversation_wakeup(true, 18, 4, 6, 5, 3);
    metrics.record_conversation_extraction_lag(11);
    metrics.record_conversation_hook_enqueue_latency(7);
    metrics.record_conversation_transcript_search_latency(9);
    metrics.set_conversation_archive_size_bytes(512);
    metrics.record_conversation_compaction(2, 1, 1, 3, 128);
    ...
    assert!(payload.contains("docdex_conversation_wakeup_requests_total 1"));
    assert!(payload.contains("docdex_conversation_wakeup_useful_total 1"));
    assert!(payload.contains("docdex_conversation_prompt_budget_saved_tokens_total 18"));
    assert!(payload.contains("docdex_conversation_extraction_lag_count_total 1"));
    assert!(payload.contains("docdex_conversation_hook_enqueue_latency_count_total 1"));
    assert!(payload.contains("docdex_conversation_transcript_search_latency_count_total 1"));
    assert!(payload.contains("docdex_conversation_archive_size_bytes 512"));
    assert!(payload.contains("docdex_conversation_compaction_runs_total 1"));
    assert!(payload.contains("docdex_conversation_compaction_deleted_sessions_total 2"));
    assert!(payload.contains("docdex_conversation_compaction_reclaimed_bytes_total 128"));
