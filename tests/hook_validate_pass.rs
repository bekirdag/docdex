...
fn write_repo(repo_root: &Path) -> Result<(), Box<dyn Error>> {
    ...
    fs::write(src_dir.join("safe.ts"), "const value: string = \"ok\";\n")?;
    Ok(())
}
...
#[test]
fn hook_validate_passes_without_violations() -> Result<(), Box<dyn Error>> {
    ...
    let resp = client
        ...
        .json(&serde_json::json!({ "files": ["src/safe.ts"] }))
        .send()?;
