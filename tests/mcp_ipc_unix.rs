...
mod unix_tests {
    ...
    #[test]
    fn mcp_ipc_unix_roundtrip() -> Result<(), Box<dyn Error>> {
        let repo = TempDir::new()?;
        write_repo(repo.path())?;

        let state_root = TempDir::new()?;
        let home_dir = TempDir::new()?;
        let socket_path = home_dir.path().join(".docdex").join("mcp.sock");
        ...
        rt.block_on(async {
            ...
            let (status, init_resp) = send_mcp_request(&socket_path, &init_payload).await?;
            ...
            let (status, tools_resp) = send_mcp_request(&socket_path, &tools_payload).await?;
            if !status.is_success() {
            ...
            let (status, search_resp) = send_mcp_request(&socket_path, &search_payload).await?;
            if !status.is_success() {
